...
NodeType = Union[TreeNode, XpNode]
...
class BuildST:
    ...
    def __init__(self, trees: Collection[Tree]) -> None:
        ...
        # leaf label --> XpNode
        self.label2xp_node: dict[Any, XpNode] = {}
        ...
        # HDT datastructure for dynamic graph connectivity
        ...
        for i in range(len(self.trees)):
            ...
            for node in tree.preorder():
                ...
                if node.label not in self.label2xp_node:
                    self.label2xp_node[node.label] = XpNode(node.label)
...
class _ConnectedComp:
    ...
    def __init__(
        ...
        self.List = [DLList() for _ in range(k)]
    ...
    def __iter__(self) -> Iterator[TreeNode | XpNode]:
    ...
    def get_label_nodes(self) -> list[XpNode]:
        """Return the label nodes in the connected component.

        Returns:
            A list of label nodes (XpNode instances) in this components
        """
        result = []
        if not self.representative:
