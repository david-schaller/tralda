...
class LooseConsensusTree:
    ...
    def run(self):
        ...
        T = self.trees[0]
        for j in range(1, k):
            A, x = one_way_compatible(T, self.trees[j], return_no_of_contractions=True)
            self.total_contracted += x
            T = merge_trees(A, self.trees[j])

        for j in range(k):
            T, x = one_way_compatible(T, self.trees[j], return_no_of_contractions=True)
            self.total_contracted += x
...
def _preprocess(T1, T2):
    ...
    f = {leaf.label: i for i, leaf in enumerate(T1.leaves())}
    ...
    L_T2 = list(T2.leaves())
    leaf_rank = {leaf.label: i for i, leaf in enumerate(L_T2)}
    start, stop = {}, {}
