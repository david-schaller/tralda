...
def aho_graph(R, L, weighted=False, triple_weights=None):
    ...
    for a, b, c in R:
        ...
        if weighted:
            if triple_weights:
                G[a][b]["weight"] = G[a][b].get("weight", 0.0) + triple_weights[a, b, c]
            else:
...
class Build:
    ...
    def __init__(self, R, L, mincut=False, weighted_mincut=False, triple_weights=None):
        self.R = R
        self.L = L
        self.mincut = mincut
        ...
        for v2 in u.children:
            if v is not v2:
                for i in range(len(W) - 1):
                    yield (repres[W[i]], repres[W[i + 1]], repres[v2])
...
def tree_profile_to_triples(trees):
    ...
    for tree in trees:
        L.update(leaf.label for leaf in tree.leaves())
        R.update((*sorted(t[:2]), t[2]) for t in minimal_identifying_triple_set(tree))
